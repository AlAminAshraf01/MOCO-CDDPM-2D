...
The maths are rank agnostic (``shift_dim`` + ``flatten``), so the only change
relative to the 3D file is that tensors are ``[b, c, h, w]`` instead of
``[b, c, t, h, w]``.
"""
...
from vq_gan_2d.utils import shift_dim
...
class Codebook(nn.Module):
    ...
    def forward(self, z):
        ...
        encode_onehot = F.one_hot(
            encoding_indices, self.n_codes).type_as(flat_inputs)  # [bhw, ncode]
        encoding_indices = encoding_indices.view(
            z.shape[0], *z.shape[2:])  # [b, h, w]

        embeddings = F.embedding(
            encoding_indices, self.embeddings)  # [b, h, w, c]
