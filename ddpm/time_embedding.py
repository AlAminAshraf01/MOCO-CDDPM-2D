"""Unchanged from the 3D repo -- time embedding is dimension agnostic."""
...
class SinusoidalPosEmb(nn.Module):
    def __init__(self, emb_dim=16, downscale_freq_shift=1, max_period=10000,
...
class LearnedSinusoidalPosEmb(nn.Module):
    """Following @crowsonkb's learned sinusoidal positional embedding."""
...
class TimeEmbbeding(nn.Module):
    def __init__(self, emb_dim=64, pos_embedder=SinusoidalPosEmb,
                 pos_embedder_kwargs={}, act_name=("SWISH", {})):
        super().__init__()
        self.emb_dim = emb_dim
        self.pos_emb_dim = pos_embedder_kwargs.get('emb_dim', emb_dim // 4)
        pos_embedder_kwargs = dict(pos_embedder_kwargs)
